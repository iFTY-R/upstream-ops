...
var ldxpItemHTTPClient = func() *http.Client {
...
func ParseShopURLContext(ctx context.Context, raw string) (*ParsedURL, error) {
	...
	for i := 0; i < len(parts)-1; i++ {
		...
		if strings.EqualFold(parts[i], "item") && strings.TrimSpace(parts[i+1]) != "" {
			goodsKey, err := LDXPItemGoodsKey(raw)
			if err != nil {
				return nil, err
			}
			return resolveLDXPItemURL(ctx, baseURL, goodsKey)
		}
	}
	return nil, fmt.Errorf("unsupported shop url")
}

// LDXPItemGoodsKey validates a persisted LDXP item URL without contacting the
// remote shop. Callers use it to decide whether an exact-item action is safe
// to expose; resolving the shop still happens through ParseShopURLContext.
func LDXPItemGoodsKey(raw string) (string, error) {
	parsed, err := url.Parse(strings.TrimSpace(raw))
	if err != nil {
		return "", err
	}
	if err := validateLDXPItemURL(parsed); err != nil {
		return "", err
	}
	parts := strings.Split(strings.Trim(parsed.Path, "/"), "/")
	if len(parts) != 2 || !strings.EqualFold(parts[0], "item") || strings.TrimSpace(parts[1]) == "" {
		return "", fmt.Errorf("ldxp item URL path must be /item/{goodsKey}")
	}
...
func resolveLDXPItemURL(ctx context.Context, baseURL, goodsKey string) (*ParsedURL, error) {
	...
	canonicalBaseURL, err := canonicalLDXPBaseURL(baseURL)
	if err != nil {
		return nil, fmt.Errorf("resolve item url: %w", err)
	}
	siteURL := canonicalBaseURL + "/shop/" + url.PathEscape(token)
	return &ParsedURL{
		Platform: storage.ShopPlatformLDXP,
		SiteURL:  siteURL,
...
func validateLDXPItemURL(parsed *url.URL) error {
	if parsed == nil || parsed.User != nil || !strings.EqualFold(parsed.Scheme, "https") {
		return fmt.Errorf("ldxp item URL must use HTTPS")
	}
	if !isAllowedLDXPItemHost(parsed.Host) {
		return fmt.Errorf("ldxp item URL host %q is not allowed", parsed.Host)
	}
	if parsed.RawQuery != "" || parsed.Fragment != "" {
		return fmt.Errorf("ldxp item URL must not contain query or fragment")
	}
	return nil
}

func canonicalLDXPBaseURL(raw string) (string, error) {
	parsed, err := url.Parse(strings.TrimSpace(raw))
	if err != nil {
		return "", err
	}
...
func isAllowedLDXPItemHost(host string) bool {
	switch strings.ToLower(strings.TrimSpace(host)) {
	case "pay.ldxp.cn", "www.ldxp.cn":
		return true
	default:
		return false
	}
